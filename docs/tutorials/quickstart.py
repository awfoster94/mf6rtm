...
# %%
sim_ws = os.path.join(BASE, "_quickstart_run")
...
#
# Two solutions (background = 1, inflow = 2) and the two equilibrium-phase
# minerals (calcite, dolomite), read from CSV.

# %%
solutionsdf = pd.read_csv(os.path.join(DATA, "ex1_solutions.csv"),
                          comment="#", index_col=0)
...
# %%
eqp_df = pd.read_csv(os.path.join(DATA, "ex1_equilibrium_phases.csv"))
...
model.set_postfix(os.path.join(DATA, "ex1_postfix.phqr"))
model.initialize()
...
# Locate the MODFLOW 6 shared library next to the ``mf6`` executable on PATH.
_libname = {"Linux": "libmf6.so", "Darwin": "libmf6.dylib",
            "Windows": "libmf6.dll"}[platform.system()]
libmf6 = os.path.join(os.path.dirname(shutil.which("mf6")), _libname)

model.write_simulation()
model.run(libname=libmf6)
