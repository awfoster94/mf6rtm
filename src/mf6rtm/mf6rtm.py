...
def solve(wd, 
          ...
          theta_immobile=None,
          zetaim=None,
          phreeqcbmi_immobile=None):
    '''Wrapper to prepare and call solve functions
    '''
    mf6rtm = initialize_interfaces(wd)
    mf6rtm.ddmt_enabled = ddmt_enabled
    mf6rtm.theta_mobile = theta_mobile
    mf6rtm.theta_immobile = theta_immobile
    mf6rtm.zetaim = zetaim
    mf6rtm.phreeqcbmi_immobile = phreeqcbmi_immobile

    if ddmt_enabled:
        if phreeqcbmi_immobile is None:
            raise ValueError("DDMT is enabled but phreeqcbmi_immobile is None.")
        if theta_mobile is None or theta_immobile is None or zetaim is None:
            raise ValueError("DDMT requires theta_mobile, theta_immobile, and zetaim.")
...
class Mf6RTM(object):
    ...
    def _prepare_to_solve(self):
        ...
        if getattr(self, "ddmt_enabled", False):
            if self.phreeqcbmi_immobile.ncomps != self.phreeqcbmi.ncomps:
                raise ValueError("Mobile and immobile PhreeqcRM instances have different ncomps.")

            if list(self.phreeqcbmi.components) != list(self.phreeqcbmi_immobile.components):
                raise ValueError("Mobile and immobile PhreeqcRM instances have different component order.")
    ...
    def _get_rm_conc_matrix(self, rm):
        c = rm.GetConcentrations()
        return np.asarray(c, dtype=float).reshape(self.phreeqcbmi.ncomps, self.nxyz)


    def _set_rm_conc_matrix(self, rm, conc):
        c = np.ascontiguousarray(conc.reshape(-1))
        rm.SetConcentrations(c)


    def _apply_ddmt_exchange(self, dt):
        cm = self.current_iteration_conc
        cim = self._get_rm_conc_matrix(self.phreeqcbmi_immobile)

        tm = np.asarray(self.theta_mobile, dtype=float).reshape(1, self.nxyz)
        ti = np.asarray(self.theta_immobile, dtype=float).reshape(1, self.nxyz)
        z = np.asarray(self.zetaim, dtype=float).reshape(1, self.nxyz)

        cm_new = cm.copy()
        cim_new = cim.copy()

        active = ((tm > 0.0) & (ti > 0.0) & (z > 0.0)).ravel()
        ...
        if np.any(active):
            ...
            cm_calc = (total + ti * diff_new) / (tm + ti)
            cim_calc = (total - tm * diff_new) / (tm + ti)

            cm_new[:, active] = cm_calc[:, active]
            cim_new[:, active] = cim_calc[:, active]

        self.current_iteration_conc = cm_new
        self._set_rm_conc_matrix(self.phreeqcbmi, cm_new)
        self._set_rm_conc_matrix(self.phreeqcbmi_immobile, cim_new)
    ...
    def _apply_ddmt_exchange(self, dt):
        ...
        mass_after = tm * cm_new + ti * cim_new

        if not np.allclose(mass_before[:, active], mass_after[:, active], rtol=1e-10, atol=1e-14):
            raise RuntimeError("DDMT exchange failed mass-conservation check.")
    ...
    def _solve(self):
        ...
        while ctime < etime:
            ...
            if self.reactive:
                ...
                # react immobile domain after ddmt exchange
                if getattr(self, "ddmt_enabled", False):
                    immobile_diffmask = np.ones(self.nxyz)
                    self.phreeqcbmi_immobile._solve_phreeqcrm(
                        dt,
                        diffmask=immobile_diffmask,
                    )
