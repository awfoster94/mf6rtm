...
class Mup3d(object):
    def __init__(self, name, solutions, nlay, nrow, ncol):
        ...
        self.ddmt_enabled = False
        self.theta_mobile = None
        self.theta_immobile = None
        self.zetaim = None
        self.phreeqc_rm_immobile = None
    ...
    def enable_ddmt_reactive_immobile(self, theta_mobile, theta_immobile, zetaim, phreeqc_rm_immobile):
        """Enable dual-domain mass transfer with reactive immobile chemistry.

        theta_* are water contents per bulk volume, shape nxyz.
        zetaim is the first-order exchange conductance, same time units as dt.
        """
        self.ddmt_enabled = True
        self.theta_mobile = np.asarray(theta_mobile, dtype=float).reshape(self.nxyz)
        self.theta_immobile = np.asarray(theta_immobile, dtype=float).reshape(self.nxyz)
        self.zetaim = np.asarray(zetaim, dtype=float).reshape(self.nxyz)
        self.phreeqc_rm_immobile = phreeqc_rm_immobile
    ...
    def run(self, reactive = True):
        ...
        return solve(self.wd,
                     reactive=reactive,
                     ddmt_enabled=self.ddmt_enabled,
                     theta_mobile=self.theta_mobile,
                     theta_immobile=self.theta_immobile,
                     zetaim=self.zetaim,
                     phreeqcbmi_immobile=self.phreeqc_rm_immobile)
